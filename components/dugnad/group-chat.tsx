...
const GroupChat = ({
        ...
        messagesContainerRef.current?.scrollTo({
          top:
            page === 1
              ...
              : messagesContainerRef.current?.scrollHeight - prevScrollTop,
        });
      }, 20);
    } catch (error) {
  ...
  const isUserAtBottom = () => {
    const messagesContainer =
      (messagesContainerRef.current as HTMLDivElement) || 0;
  ...
  return (
    <div className="bg-gray-200 py-4 p-3 relative rounded-md">
      ...
      <div
        ...
        className="h-[400px] overflow-scroll overflow-x-hidden flex flex-col gap-2 chat-scrollbar"
        onScroll={() => {
          if (
          ...
          <div key={index}>
            {message.ownerId === userId ? (
              <div className="flex flex-col items-end mr-1">
                <div
                  className={`rounded-md p-2.5 w-fit max-w-[70%] bg-black text-white`}
                >
              ...
              <>
                ...
                <div
                  className={`rounded-md p-2.5 w-fit max-w-[70%] bg-green-400`}
                >
                  <p className="break-words">{message.message}</p>
                </div>
              </>
      ...
      {dugnadStatus === "ACTIVE" && (
        <div className="relative mt-3">
          ...
          />
          <button
            className={`absolute right-3 top-1/2 -translate-y-[60%] ${
              input.trim() ? "text-gray-800" : "text-gray-400 cursor-default"
            }`}
            onClick={sendMessage}
          >
            <SendHorizontal />
